    ...
    $(document).on('click', '.gift-wrap-btn', function () {
        ...
        .done(function (response) {
            if (response.success) {
                ...
                if (action === 'add') {
                    $btn.text('Remove ' + button_text).addClass('added').attr('data-action', 'remove');
                } else {
                    $btn.text('Add ' + button_text).removeClass('added').attr('data-action', 'add');
                }
                ...
                $btn.prop('disabled', false).text(
                    action === 'add' ? 'Add ' + button_text : 'Remove ' + button_text
                );
            }
        })
        .fail(function () {
